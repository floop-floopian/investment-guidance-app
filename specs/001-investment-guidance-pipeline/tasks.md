...
- [ ] T002 Add all dependencies to pyproject.toml: praw, feedparser, finnhub-python, alpha-vantage, apscheduler, supabase, typer, fastapi, uvicorn, pydantic, pydantic-settings, pytest, pytest-asyncio
...
- [ ] T004 [P] Implement Pydantic BaseSettings config in src/config/settings.py covering all env vars: GROQ_API_KEY, FINNHUB_API_KEY, ALPHA_VANTAGE_API_KEY, REDDIT_CLIENT_ID, REDDIT_CLIENT_SECRET, REDDIT_USER_AGENT, DISCORD_BOT_TOKEN, SUPABASE_URL, SUPABASE_KEY, STATE_LOG_PATH, CAPITAL_MIN_POSITION_USD, BARBELL_SAFE_CORE_RATIO (default 0.60), BARBELL_SAFE_BETA_MAX, BARBELL_SATELLITE_BETA_MIN, SENTIMENT_CRITICAL_DELTA, MONITOR_INTERVAL_HIGH_VOL_MINUTES, MONITOR_INTERVAL_MED_VOL_MINUTES, MONITOR_INTERVAL_LOW_VOL_MINUTES
...
**Goal**: User provides capital, triggers pipeline, receives Discord message
...
without error, Discord message received containing ≥1 stock, risk-reward score,
...
- [ ] T016 [US1] Write failing integration test for end-to-end pipeline in tests/integration/test_pipeline_end_to_end.py: mock all adapters, assert StateLogEntry written before Discord call, assert allocation sum ≤ capital, assert all NDJSON entries present
...
- [ ] T019 [US1] Implement SentimentService in src/services/sentiment_service.py: batch all ingested items into single Groq API call (llama-3.3-70b-versatile), parse structured JSON response, update sentiment_score and sentiment_label on each MacroSignal, compute aggregate signal
...
- [ ] T024 [US1] Implement ShortlistService in src/services/shortlist_service.py: filter EXCLUDED stocks, compute risk_reward_score (weighted composite of fundamentals + technicals + macro signal), call Groq API for per-stock reasoning text, return sorted shortlist
- [ ] T025 [US1] Implement AllocationService in src/services/allocation_service.py: split shortlist by barbell band, apply configurable capital ratio (BARBELL_SAFE_CORE_RATIO, default 0.60 safe-core / 0.40 satellite), proportional weight by risk_reward_score, enforce minimum position size, call Groq API for per-allocation and overall rationale, return List[Allocation]
- [ ] T026 [P] [US1] Implement DiscordAdapter in src/adapters/discord_adapter.py: aiohttp async send_message via Discord Bot API to each active user's DM channel (resolved from Supabase `users` table), format shortlist + allocation table as Markdown, 2-retry backoff on failure
...
**Checkpoint**: User Story 1 fully functional — run-pipeline delivers Discord
...
**Goal**: System polls macro sources on a configurable schedule; sends Discord
...
if signal detected, confirm Discord received within 60s.
...
MACRO_INGESTION_COMPLETE, SENTIMENT_SCORED, ALLOCATION_GENERATED, DISCORD_SENT.
...
- [ ] T036 [P] [US3] Write failing integration tests for state log completeness in tests/integration/test_state_log.py: run mock pipeline, assert NDJSON file contains entries for every ActionType in the on-demand flow in correct order, assert StateLogEntry written before Discord dispatch
...
- [ ] T038 [US3] Audit src/pipeline/orchestrator.py and src/pipeline/stages.py: verify StateLogEntry is written for every ActionType defined in data-model.md; add any missing log writes; confirm state log write always precedes Discord call (Principle VI)
...
- [ ] T041 Harden DiscordAdapter in src/adapters/discord_adapter.py: 2 retries with exponential backoff, write DISCORD_FAILED StateLogEntry on final failure, ensure failed delivery does not crash the pipeline
...
- [ ] T045 [P] Add `status` command to src/cli.py: reads last PipelineRun from Supabase, prints run_id, status, started_at, completed_at, shortlist_count, allocation_count, discord_sent
...
Task: "Implement DiscordAdapter in src/adapters/discord_adapter.py"   # T026
```
...
6. **STOP and VALIDATE**: `run-pipeline --capital 10000` → Discord received
...
- Every state log write MUST precede any Discord call (Principle VI)
